...
export default function Destinations() {
  ...
  const load = () => {
    api.get('/admin/destinations').then((r) => setList(r.data || [])).catch(() => setList([])).finally(() => setLoading(false));
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editing.id) {
        ...
        await api.put(`/admin/destinations/${editing.id}`, formData);
      } else {
        const formData = new FormData();
        formData.append('name', name);
        if (photoFile) formData.append('photo', photoFile);
  ...
  const handleDrop = async (e, dropIndex) => {
    ...
    setReordering(true);
    try {
      await api.patch('/admin/destinations/reorder', { ids: newList.map((x) => x.id) });
    } catch (err) {
      load();
  ...
  return (
    <>
      ...
      <p className={styles.desc}>Направления отображаются на главном экране мобильного приложения. Можно изменить название и фото.</p>
      <div style={{ marginBottom: '1rem' }}>
        <button type="button" className={styles.btn} onClick={openAdd}>Добавить направление</button>
      </div>
      ...
      <div className={styles.tableWrap}>
        ...
        {list.length === 0 && <p className={styles.empty}>Нет направлений. Добавьте первое.</p>}
      </div>

      {editing && (
        <Modal title={editing.id ? 'Редактировать направление' : 'Добавить направление'} onClose={() => setEditing(null)}>
